...
- (void)viewDidAppear:(BOOL)animated {
	...
	if (array) {
		...
		for (HighscoreDictionary *record in highscoresArray.arrayOfHighscoreDictionaries) {
			...
			[[self.highscoresTextView textStorage] appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"%ld points! ", record.score]]];
			[[self.highscoresTextView textStorage] appendAttributedString:[[NSAttributedString alloc] initWithString:[NSString stringWithFormat:@"Game started at %@ and lasted for %ld seconds.\n", [NSDateFormatter localizedStringFromDate:record.startTime dateStyle:NSDateFormatterShortStyle timeStyle:NSDateFormatterShortStyle], record.duration]]];
		}
...
- (NSRange)orderAndGetTopScoreRange:(HighscoresArray *)array {
	...
	NSRange range = [self.highscoresTextView.text rangeOfString:[NSString stringWithFormat:@" %ld points", record.score]];
...
- (NSRange)orderAndGetShortestRange:(HighscoresArray *)array {
	...
	HighscoreDictionary *record = array.arrayOfHighscoreDictionaries[0];
	NSRange range = [self.highscoresTextView.text rangeOfString:[NSString stringWithFormat:@" %ld seconds", record.duration]];
